...
(defun org-capture-ref-get-bibtex-gnu-org-manual ()
  "Parse gnu.org manual page."
  (when-let ((link (org-capture-ref-get-bibtex-field :url)))
    (when (string-match "gnu\\.org.+manual" link)
      (org-capture-ref-set-bibtex-field :author org-capture-ref-placeholder-value)
      (org-capture-ref-set-bibtex-field :year org-capture-ref-placeholder-value)
      (org-capture-ref-set-bibtex-field :title (org-capture-ref-query-dom :tag 'h2)))))
...
(defun org-capture-ref-get-bibtex-bilibili ()
  "Parse Youtube watch link and generate bibtex entry."
  (when-let ((link (org-capture-ref-get-bibtex-field :url)))
    (when (string-match "bilibili\\.com/video" link)
      ...
      ;; Find title.  Drop "bilibili" name.
      (org-capture-ref-set-bibtex-field :title (replace-regexp-in-string "_哔哩哔哩_bilibili" "" (org-capture-ref-query-meta 'title)))
      ;; Find year
      (org-capture-ref-set-bibtex-field
       :year
       (org-capture-ref-query-dom :class "video-info-container"
...
(defun org-capture-ref-get-bibtex-amazon ()
  ...
  (when-let ((link (org-capture-ref-get-bibtex-field :url)))
    (when (string-match-p "amazon\\." link)
      (org-capture-ref-set-bibtex-field :url link)
      (when-let ((isbn-line (seq-find (lambda (str) (string-match-p "ISBN-10" str))
                                      (mapcar #'dom-texts
	  ...
	  (or (when (org-capture-ref-get-bibtex-field :author)
          ...
          (let ((full-name (or (when (org-capture-ref-get-bibtex-field :journal)
            ...
            (let ((repl (or (cdr (car (cl-member-if (lambda (el) (string= full-name (format "[%s] " (car el)))) org-capture-ref-journal-abbreviations)))
			    (when (boundp 'org-ref-bibtex-journal-abbreviations)
