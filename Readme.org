...
The web-page contents loaded in qutebrowser can be reused by org-capture-ref without a need to load the page again for parsing. This also means that content requiring authorisation can be parser by the package.

If one wants to use this feature, extra argument =qutebrowser-fifo= will need to be provided to org-protocol from qutebrowser userscript.
